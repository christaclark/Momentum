	...
	if( $pages == '' ) {
	 ...
	 if(1 != $pages)
	 {
	 	if ( ( $smof_data['blog_pagination_type'] == 'Infinite Scroll' && is_home() ) || 
	 		 ( $smof_data['grid_pagination_type'] == 'Infinite Scroll' && ( is_page_template( 'portfolio-one-column-text.php' ) || is_page_template( 'portfolio-one-column.php' ) || is_page_template( 'portfolio-two-column.php' ) || is_page_template( 'portfolio-two-column-text.php' ) || is_page_template( 'portfolio-three-column.php' ) || is_page_template( 'portfolio-three-column-text.php' ) || is_page_template( 'portfolio-four-column.php' ) || is_page_template( 'portfolio-four-column-text.php' ) || is_page_template( 'portfolio-five-column.php' ) || is_page_template( 'portfolio-five-column-text.php' ) ||is_page_template( 'portfolio-six-column.php' ) || is_page_template( 'portfolio-six-column-text.php' ) ) )
	 	) {
		...
		if( $smof_data['post_meta'] ) {
			...
			if( ( $layout == 'alternate' || $layout == 'grid_timeline' ) &&
			 ...
			 if( ! $smof_data['post_meta_author'] ) { 
				...
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					...
					$metadata .= sprintf( '%s <span class="fn">%s</span><span class="sep">|</span>', __('By', 'Avada'), $author_post_link );
				}
			 }
			 ...
			 if( ! $smof_data['post_meta_date'] ) { 
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					...
					$metadata .= sprintf( '<span class="updated" style="display:none;">%s</span><span class="published">%s</span><span class="sep">|</span>', get_the_modified_time( 'c' ), get_the_time( $smof_data['date_format'] ) );
				}
			 }	
			...
			if( $layout == 'single' ) {
				...
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					...
					$vcard .= sprintf( '<div class="vcard">%s</div>', $metadata );
				}
				$html .= sprintf ( '<div class="meta-info">%s</div>', $vcard );				
				...
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					...
					$html .= sprintf( '<p class="single-line-meta vcard">%s</p>', $metadata );
				}
			} else {
				if( $smof_data['disable_date_rich_snippet_pages'] ) {
					$html .= sprintf( '<div class="alignleft">%s</div>', $metadata );
				} else {
					$html .= sprintf( '<div class="alignleft vcard">%s</div>', $metadata );
				}
			}
		}
